...
export function withAuth(handler: Handler) {
  return async (req: NextRequest, context?: any) => {
    try {
      ...
      const response = await handler(req, supabase, authUser.id, context);
...
const PAYMENT_STAFF_ROLES = ["admin", "accountant"];
 ...
 * - staff (admin/accountant) may pay for anyone in the org
...
export async function resolvePaymentAccess(
  ...
  if (!role || !["patient", "admin", "accountant", "doctor", "nurse"].includes(role)) {
