...
export default function ReportsPage() {
  ...
  const kpis = useMemo(() => {
    ...
    const medRev = (invoices || [])
      .filter((i) => i.status !== "cancelled" && i.status !== "void" && i.status !== "draft" && i.status !== "refunded" && inMonth(new Date(i.issue_date)))
    ...
    const prevMonthRev = (invoices || [])
      .filter((i) => i.status !== "cancelled" && i.status !== "void" && i.status !== "draft" && i.status !== "refunded" && inPrev(new Date(i.issue_date)))
      .reduce((sum, i) => sum + i.total, 0)
      + otherIncomeData
  ...
  const revenueTrendData = useMemo(() => {
    ...
    (invoices || []).filter((i) => i.status === "paid" || i.status === "partially_paid").forEach((i) => {
      ...
      const key = d.toLocaleString("default", { month: "short", year: "2-digit" });
      if (byMonth[key] !== undefined) byMonth[key] += i.total;
    });
  ...
  const recentActivity = useMemo(() => {
    ...
    if (invoices && invoices.length > 0) {
      ...
      if (paid) {
        items.push({ action: "Payment received", detail: `${formatCurrency(paid.total)} — ${paid.invoice_number}`, time: "Recently" });
      }
    }
    items.push({
    ...
    <div className="space-y-5 print:p-0">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <DropdownMenuContent align="end" className="w-44 border-white/[0.06] bg-card backdrop-blur-xl text-foreground/80">
