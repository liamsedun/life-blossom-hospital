...
interface DutyShift {
  ...
  until_time: string;
  note: string | null;
  staff?: { id: string; employee_code: string; department_id?: string | null; user?: { id: string; first_name: string; last_name: string } | null };
}
...
const roles = ["doctor", "nurse", "admin", "accountant", "cashier", "receptionist", "lab_technician", "pharmacist", "radiographer", "radiologist"];
...
export default function StaffPage() {
  ...
  const departments = useMemo(() => {
    ...
    const depts = new Set(staffData.map((s) => s.department_id || "General"));
  ...
  function openEdit(s: Staff) {
    ...
    setForm({
      ...
      specialization: s.specialty || "", department: s.department_id || "",
      license_number: "",
    });
    setFormError("");
  }
  ...
  const filteredRoster = useMemo(() => {
    ...
    return rosterShifts.filter((s) => {
      const name = s.staff?.user ? `${s.staff.user.first_name} ${s.staff.user.last_name}` : (s.staff?.employee_code || "");
    ...
    return (
      <div className="flex items-center gap-1">
        <button type="button" onClick={() => openEditShift(s)}
          ...
          className="p-1.5 rounded-lg text-muted-foreground hover:text-[#e0a84a] hover:bg-muted transition-colors">
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input placeholder="Search by name, department, or specialty..."
            className="h-9 pl-9 text-sm bg-background border-border text-foreground placeholder:text-muted-foreground focus-visible:border-[#e0a84a]/40 focus-visible:ring-[#e0a84a]/20"
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {loading ? (
          Array.from({ length: 6 }).map((_, i) => (
            <Card key={i} className="border-border bg-card backdrop-blur-xl">
          ...
          const name = staff.user ? `${staff.user.first_name} ${staff.user.last_name}` : staff.employee_code;
          ...
          return (
            <Card key={staff.id} className="border-border bg-card backdrop-blur-xl hover:border-accent transition-all group">
              <CardContent className="p-5">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground capitalize">{staff.user?.role?.replace("_", " ") || "Staff"}</p>
                    </div>
                  </div>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon" className="size-8 opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-foreground">
                        <MoreHorizontal className="size-4" />
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-44 border-border bg-card backdrop-blur-xl text-foreground">
                ...
                <div className="mt-4 space-y-2 text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>{staff.department_id}{staff.specialty ? ` — ${staff.specialty}` : ""}</span>
                    </div>
                  )}
                  <div className="flex items-center gap-2">
      ...
      <Card className="border-border bg-card backdrop-blur-xl">
        <CardHeader className="pb-2 flex flex-row items-center justify-between flex-wrap gap-3">
          <CardTitle className="text-base text-foreground">Duty Roaster</CardTitle>
          <Button variant="ghost" size="sm" className="h-7 text-xs text-[#e0a84a]/70 hover:text-[#e0a84a]"
            onClick={() => openRoster()}>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="relative flex-1 min-w-[180px] max-w-xs">
              ...
              <Input placeholder="Filter by staff name..."
                className="h-9 pl-9 text-sm bg-background border-border text-foreground placeholder:text-muted-foreground"
            ...
            <div className="flex items-center gap-1.5">
              ...
              <input type="date" value={rosterFrom} onChange={(e) => e.target.value && setRosterFrom(e.target.value)}
                className="h-9 rounded-xl border border-border bg-background px-2.5 text-xs text-foreground" />
              ...
              {([
                ...
                <button key={key} onClick={() => setRosterView(key)}
                  className={cn("flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg transition-all",
                    rosterView === key ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                  )}>
                ...
                <tbody className="divide-y divide-border">
                  {filteredRoster.map((s) => (
                    <tr key={s.id} className="hover:bg-muted/50 transition-colors">
                      <td className="px-5 py-2.5 text-foreground whitespace-nowrap">{fmtDate(s.shift_date)}</td>
                      <td className="px-5 py-2.5 text-foreground font-medium whitespace-nowrap">{shiftName(s)}</td>
                      <td className="px-5 py-2.5 text-muted-foreground">{s.staff?.department_id || "—"}</td>
                      <td className="px-5 py-2.5">{shiftInfo(s)}</td>
                      <td className="px-5 py-2.5 text-muted-foreground max-w-[180px] truncate">{s.note || "—"}</td>
                ...
                <div key={staffId} className="px-5 py-4">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-foreground">{shiftName(shifts[0])}</p>
                      <p className="text-[11px] text-muted-foreground">{shifts[0]?.staff?.department_id || "—"} · {shifts.length} shift(s)</p>
                    </div>
                  </div>
                  <div className="grid gap-2 sm:grid-cols-2">
                    {shifts.map((s) => (
                      <div key={s.id} className="flex items-center justify-between gap-2 rounded-xl border border-border bg-background px-3 py-2">
                        <div>
                          <p className="text-xs text-foreground">{fmtDate(s.shift_date)}</p>
            ...
            <div className="divide-y divide-border">
              {rosterByDay.map(([date, shifts]) => (
                <div key={date} className="px-5 py-4">
                  <p className="text-sm font-semibold text-foreground mb-2">{fmtDate(date)} <span className="text-muted-foreground font-normal text-xs">· {shifts.length} shift(s)</span></p>
                  <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {shifts.map((s) => (
                      <div key={s.id} className="flex items-center justify-between gap-2 rounded-xl border border-border bg-background px-3 py-2">
                        <div className="min-w-0">
                          <p className="text-xs text-foreground truncate">{shiftName(s)}</p>
                          <p className="text-[11px] text-muted-foreground">{s.staff?.department_id || "—"}</p>
                          <p className="text-[11px] text-[#e0a84a] mt-0.5">{shiftInfo(s)}</p>
                        </div>
                        {shiftRowActions(s)}
                      </div>
              ...
              {rosterByWeek.map(([mondayStr, shifts]) => (
                <div key={mondayStr} className="px-5 py-4">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-sm font-semibold text-foreground">{fmtWeekRange(mondayStr)}</p>
                    <span className="text-[11px] text-muted-foreground">{shifts.length} shift(s)</span>
                  </div>
                  <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {shifts.map((s) => (
                      <div key={s.id} className="flex items-center justify-between gap-2 rounded-xl border border-border bg-background px-3 py-2">
                        <div className="min-w-0">
                          <p className="text-xs text-foreground">{fmtDate(s.shift_date)}</p>
                          <p className="text-xs text-muted-foreground truncate">{shiftName(s)}</p>
                          <p className="text-[11px] text-[#e0a84a] mt-0.5">{shiftInfo(s)}</p>
                        </div>
                        {shiftRowActions(s)}
                      </div>
      ...
      <Dialog open={showAdd} onOpenChange={(o) => { if (!creating) { setShowAdd(o); if (!o) resetForm(); } }}>
        <DialogContent className="sm:max-w-md border-border bg-card backdrop-blur-xl text-foreground">
          <DialogHeader><DialogTitle className="text-foreground">Add Staff Member</DialogTitle></DialogHeader>
          <form onSubmit={handleAdd} className="space-y-4">
            ...
            <div>
              ...
              <Input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })}
                placeholder="Min 6 characters" className="bg-background border-border text-foreground" required minLength={6} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}
                  ...
                  {availableRoles.map((r) => (
                    <option key={r} value={r} className="bg-card">{r.replace("_", " ").replace(/\b\w/g, (c) => c.toUpperCase())}</option>
                  ))}
                </select>
              </div>
              <div>
                ...
                <Input value={form.department} onChange={(e) => setForm({ ...form, department: e.target.value })}
                  placeholder="e.g. Cardiology" className="bg-background border-border text-foreground" />
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Specialization</label>
              <Input value={form.specialization} onChange={(e) => setForm({ ...form, specialization: e.target.value })}
                placeholder="e.g. Cardiologist" className="bg-background border-border text-foreground" />
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Phone</label>
              <Input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}
                placeholder="+234 XXX XXX XXXX" className="bg-background border-border text-foreground" />
            </div>
      ...
      <Dialog open={!!availStaff} onOpenChange={(o) => { if (!o && !saving) setAvailStaff(null); }}>
        <DialogContent className="sm:max-w-sm border-border bg-card backdrop-blur-xl text-foreground">
          <DialogHeader><DialogTitle className="text-foreground">Availability</DialogTitle></DialogHeader>
          {availStaff && (
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-3 bg-muted rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{availStaff.department_id || "—"}</p>
                </div>
              </div>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm text-foreground">{scheduleForm.is_available ? "Available for duty" : "Not available"}</span>
              </label>
      ...
      <Dialog open={showRoster} onOpenChange={(o) => { if (!o && !rosterSaving) setShowRoster(false); }}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto border-border bg-card backdrop-blur-xl text-foreground">
          <DialogHeader><DialogTitle className="text-foreground">Schedule Duty (Roaster)</DialogTitle></DialogHeader>
          <div className="space-y-4">
            {/* Staff selection */}
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-xs font-medium text-muted-foreground">Staff ({rosterForm.staff_ids.length} selected)</label>
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => setRosterForm((p) => ({ ...p, staff_ids: [] }))}
                    className="text-[10px] font-medium text-muted-foreground hover:text-foreground">Clear</button>
                </div>
              </div>
                  ...
                  return (
                    <button key={s.id} type="button" onClick={() => toggleStaff(s.id)}
                      ...
                      <div className={cn("size-4 rounded border flex items-center justify-center shrink-0",
                        checked ? "bg-[#e0a84a] border-[#e0a84a]" : "border-muted-foreground/30")}>
                      ...
                      <span className="text-xs text-foreground">{s.user?.first_name} {s.user?.last_name}</span>
                      <span className="ml-auto text-[10px] text-muted-foreground capitalize">{s.department_id || "—"}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">FROM (time)</label>
                <Input type="time" value={rosterForm.from_time}
                  onChange={(e) => setRosterForm({ ...rosterForm, from_time: e.target.value })}
                  className="bg-background border-border text-foreground" />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">UNTIL (time)</label>
            ...
            <div>
              ...
              <Input value={rosterForm.note}
                ...
                placeholder="e.g. Ward A shift" className="bg-background border-border text-foreground" />
            </div>
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm text-foreground">Notify staff (in-app + push): "DATE: … TIME: FROM: … UNTIL: …"</span>
            </label>

            <div className="rounded-xl bg-muted border border-border px-3 py-2.5 text-xs text-muted-foreground">
              {rosterTotalShifts > 0
                ? <><span className="text-foreground font-medium">{rosterTotalShifts} shift(s)</span> across {rosterDays} day(s) for {rosterForm.staff_ids.length} staff member(s)</>
      ...
      <Dialog open={!!leaveStaff} onOpenChange={(o) => { if (!o && !leaveSaving) setLeaveStaff(null); }}>
        <DialogContent className="sm:max-w-sm border-border bg-card backdrop-blur-xl text-foreground">
          <DialogHeader><DialogTitle className="text-foreground">Mark On Leave</DialogTitle></DialogHeader>
          {leaveStaff && (
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-3 bg-muted rounded-xl">
                <Avatar className="size-10 ring-1 ring-[#e0a84a]/20">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {leaveStaff.on_leave_until ? `Currently on leave until ${fmtDate(leaveStaff.on_leave_until)}` : (leaveStaff.department_id || "—")}
                  </p>
                </div>
              </div>

              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Returns to Duty On</label>
      ...
      <Dialog open={!!editShift} onOpenChange={(o) => { if (!o && !editSaving) setEditShift(null); }}>
        <DialogContent className="sm:max-w-sm border-border bg-card backdrop-blur-xl text-foreground">
          <DialogHeader><DialogTitle className="text-foreground">Reschedule / Edit Duty Shift</DialogTitle></DialogHeader>
          {editShift && (
            <form onSubmit={handleEditShift} className="space-y-4">
              <div className="flex items-center gap-3 p-3 bg-muted rounded-xl">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-foreground truncate">{shiftName(editShift)}</p>
                  <p className="text-xs text-muted-foreground">{editShift.staff?.department_id || "—"}</p>
                </div>
              </div>

              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Duty Date</label>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm text-foreground">Notify staff of the change</span>
              </label>
      ...
      <Dialog open={!!deleteShift} onOpenChange={(o) => { if (!o && !deleteShiftLoading) setDeleteShift(null); }}>
        <DialogContent className="sm:max-w-sm border-border bg-card backdrop-blur-xl text-foreground">
          <DialogHeader><DialogTitle className="text-foreground">Delete Duty Shift</DialogTitle></DialogHeader>
          {deleteShift && (
            <>
              <p className="text-sm text-muted-foreground">
                Remove the duty shift for <strong className="text-foreground">{shiftName(deleteShift)}</strong> on{" "}
                <strong className="text-foreground">{fmtDate(deleteShift.shift_date)}</strong> ({fmtTime(deleteShift.from_time)} – {fmtTime(deleteShift.until_time)})?
              </p>
              <DialogFooter>
                <DialogClose asChild>
                  <Button type="button" variant="outline" className="bg-white text-black border-border hover:bg-gray-100">Cancel</Button>
      ...
      <Dialog open={!!deleteStaff} onOpenChange={(o) => { if (!o) setDeleteStaff(null); }}>
        <DialogContent className="sm:max-w-sm border-border bg-card backdrop-blur-xl text-foreground">
          <DialogHeader><DialogTitle className="text-foreground">Deactivate Staff</DialogTitle></DialogHeader>
          <p className="text-sm text-muted-foreground">
