...
export default function AdminChatsPage() {
  ...
  return (
    <div className="flex items-stretch gap-4 h-[calc(100dvh-170px)] lg:h-[calc(100dvh-120px)]">
      {/* Conversation list */}
      <Card className={`w-full lg:w-80 shrink-0 overflow-hidden border-white/[0.06] bg-card ${activeChatId ? "hidden lg:flex" : "flex"}`}>
      ...
      {/* Window */}
      <Card className={`flex-1 overflow-hidden border-white/[0.06] bg-card ${activeChatId ? "flex" : "hidden lg:flex"}`}>
        {activeChatId ? (
