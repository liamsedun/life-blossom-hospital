...
type IncomeEntry = {
  ...
  source: string | null; notes: string | null; bank_account_id: string | null;
...
interface IncomeForm {
  ...
  payment_method: string; source: string; notes: string; bank_account_id: string;
}
...
const emptyForm: IncomeForm = {
  ...
  payment_method: "cash", source: "", notes: "", bank_account_id: "",
};
...
export default function OtherIncomePage() {
  ...
  useEffect(() => {
    fetch("/api/settings/bank-accounts")
      .then((r) => r.json())
      .then((json) => { if (json.success) setBankAccounts((json.data?.accounts || []).filter((a: BankAccount) => a.is_active)); })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="space-y-5">
                ...
                {filtered.length === 0 ? (
                  ...
                  return (
                    <tr key={item.id} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-5 py-3 font-medium text-white text-xs">{item.description}</td>
        ...
        <DialogContent className="sm:max-w-md border-white/[0.06] bg-card backdrop-blur-xl text-foreground">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select value={form.payment_method} onChange={(e) => setForm({ ...form, payment_method: e.target.value })}
                  ...
                  {PAYMENT_METHODS.map((m) => (
                    <option key={m} value={m} className="bg-card">{m.replace("_", " ")}</option>
                  ))}
                </select>
              </div>
            </div>
            {bankAccounts.length > 0 && (
              <div>
                <label className="block text-xs font-medium text-white/50 mb-1">Deposit To (Bank Account)</label>
                <select value={form.bank_account_id} onChange={(e) => setForm({ ...form, bank_account_id: e.target.value })}
                  className="flex h-9 w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-3 py-2 text-sm text-white">
                  <option value="" className="bg-card">Select bank account...</option>
                  {bankAccounts.map((a) => (
                    <option key={a.id} value={a.id} className="bg-card">{a.bank_name} — {a.account_name} ({a.account_number})</option>
                  ))}
                </select>
              </div>
            )}
      ...
      <Dialog open={!!deleteItem} onOpenChange={(o) => { if (!o) setDeleteItem(null); }}>
        <DialogContent className="sm:max-w-sm border-white/[0.06] bg-card backdrop-blur-xl text-foreground">
