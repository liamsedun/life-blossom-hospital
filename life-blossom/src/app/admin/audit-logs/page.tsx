...
const ROLES = ["admin", "doctor", "nurse", "accountant", "patient"];
            ...
            {tab === "logs" ? (
              <>
                <select value={entityType} onChange={(e) => setEntityType(e.target.value)} className={selectCls}>
                  <option value="" className="bg-card">All Tables</option>
                  {ENTITIES.map((t) => <option key={t} value={t} className="bg-card">{t}</option>)}
                </select>
                <select value={action} onChange={(e) => setAction(e.target.value)} className={selectCls}>
                  ...
                  {ACTIONS.map((a) => <option key={a} value={a} className="bg-card">{a}</option>)}
                </select>
                ...
                <select value={userId} onChange={(e) => setUserId(e.target.value)} className={cn(selectCls, "max-w-[220px]")}>
                  <option value="" className="bg-card">All Users</option>
                  {users.map((u) => <option key={u.id} value={u.id} className="bg-card">{u.label}</option>)}
                </select>
              </>
            ) : (
              <>
                <select value={eventType} onChange={(e) => setEventType(e.target.value)} className={selectCls}>
                  <option value="" className="bg-card">All Event Types</option>
                  {EVENT_TYPES.map((t) => <option key={t} value={t} className="bg-card">{t}</option>)}
                </select>
                <select value={severity} onChange={(e) => setSeverity(e.target.value)} className={selectCls}>
                  ...
                  {SEVERITIES.map((s) => <option key={s} value={s} className="bg-card">{s}</option>)}
                </select>
              </>
            )}
