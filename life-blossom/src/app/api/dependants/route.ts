...
async function attachInvoiceSummary(svc: any, dependants: Dependant[]): Promise<Dependant[]> {
  ...
  const { data: invoices, error } = await svc
    .from("invoices")
    .select("patient_id, total, paid_amount, status")
    .in("patient_id", ids)
  ...
  for (const inv of invoices) {
    ...
    entry.outstanding += (inv.total || 0) - (inv.paid_amount || 0);
    entry.count += 1;
...
//   Patient caller  → adds dependant to their own family account
//   Staff / admin   → ?patient_id= (adds dependant to a given patient's family)
export const POST = withAuth(async (req, supabase, authUserId) => {
  ...
  const { data: caller } = await supabase.from("users").select("role").eq("id", authUserId).single();
  const isStaff = caller?.role && caller?.role !== "patient";

  // Resolve the family account to add the dependant to
  let familyPatientId: string | null = null;
  if (isStaff) {
    // Staff/admin must provide patient_id
    familyPatientId = body.patient_id || null;
    if (!familyPatientId) return err("patient_id is required for staff to add dependants", 400);
  } else {
    // Patient caller → their own family account
    familyPatientId = await resolvePatientId(supabase, authUserId);
  ...
  // Patients: only a main account holder can add dependants — dependants cannot
  // create further dependants under themselves
  if (!isStaff && primary.primary_account_id) {
  ...
  // Create a stub auth user (required by the users.id → auth.users.id FK)
  // Password meets Supabase complexity: upper, lower, digit, symbol
  const stubPassword = `Dep#${crypto.randomUUID().slice(0, 8)}!Aa1`;
  ...
  const depUserId = authData.user.id;
  ...
  if (userError) {
    ...
    await svc.auth.admin.deleteUser(depUserId).catch(() => {});
    return err(userError.message, 500);
  }
  ...
  if (patientError) {
    await svc.from("users").delete().eq("id", depUserId);
  ...
  if (body.avatar) {
    ...
    const { error: avatarError } = await svc.from("users").update({ avatar_url: avatarUrl }).eq("id", depUserId);
