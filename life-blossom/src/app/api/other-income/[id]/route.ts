...
export const PUT = withAuth(async (req, supabase, authUserId, context) => {
  ...
  const allowed = ["description", "category", "amount", "income_date", "payment_method", "source", "notes", "bank_account_id"];
  ...
  logAudit(req, authUserId, { action: "update", entityType: "other_income", entityId: id, description: "Updated income", changes: updates }).catch(() => {});
  return ok(data);
});
...
export const DELETE = withAuth(async (req, supabase, authUserId, context) => {
  ...
  if (error) return err(error.message, 500);
  logAudit(req, authUserId, { action: "delete", entityType: "other_income", entityId: id, description: "Deleted income" }).catch(() => {});
  return ok(null);
});
