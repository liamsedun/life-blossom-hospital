...
export const POST = withAuth(async (req, supabase, authUserId, context) => {
  ...
  // Only admin can reset another admin's password
  if (target.role === "admin" && caller.role !== "admin") {
    return err("Only admin can reset another admin's password", 403);
  }
  ...
  // Try to use service role client for admin auth operations
  let svc;
  try {
    svc = createServiceClient();
  } catch {
    return err("Service role key not configured. Contact the system administrator to reset passwords.", 503);
  }

  // Optional: set/replace the login email
  ...
  if (updateError) {
    if (/user.*not found|not found|no user/i.test(updateError.message || "")) {
      ...
      try {
        ...
        return err("Service role key not configured for provisioning", 503);
      }
    }
