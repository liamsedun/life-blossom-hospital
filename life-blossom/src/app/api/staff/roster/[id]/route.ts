...
export const PUT = withAuth(async (req, supabase, authUserId, ctx) => {
  ...
  const { data: existing, error: fetchErr } = await svc
    .from("duty_roster")
    .select("*, staff:staff(id, org_id, user_id, employee_code, department_id, user:users(id, first_name, last_name))")
    .eq("id", id)
    .eq("org_id", orgId)
    .maybeSingle();
  ...
  const { data: updated, error: updErr } = await svc
    ...
    .eq("id", id)
    .eq("org_id", orgId)
    .select("*, staff:staff(id, employee_code, department_id, user:users(id, first_name, last_name))")
    .single();
