...
export const GET = withAuth(async (req, supabase, authUserId) => {
  ...
  let query = supabase
    .from("duty_roster")
    .select("*, staff:staff!inner(id, employee_code, department_id, user:users(id, first_name, last_name, avatar_url, role, email))")
    .eq("org_id", orgId);
  ...
  if (department && department !== "All") query = query.eq("staff.department_id", department);
...
export const POST = withAuth(async (req, supabase, authUserId) => {
  ...
  const { data: caller } = await supabase.from("users").select("role").eq("id", authUserId).single();
  if (!caller || !["admin", "accountant"].includes(caller.role)) {
  ...
  logAudit(req, authUserId, { action: "create", entityType: "duty_roster", description: `Created ${created.length} roster entries` }).catch(() => {});
