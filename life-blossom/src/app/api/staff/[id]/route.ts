...
export const PUT = withAuth(async (req, supabase, authUserId, context) => {
  ...
  // Check caller's role - only admin can promote to admin
  if (body.role === "admin") {
    const { data: caller } = await supabase.from("users").select("role").eq("id", authUserId).single();
    if (caller?.role !== "admin") {
      throw new Error("Only admin can promote to admin");
    }
  }
  ...
  logAudit(req, authUserId, { action: "update", entityType: "staff", entityId: id, description: "Updated staff member", changes: staffFields }).catch(() => {});
  return ok(data);
});
...
export const DELETE = withAuth(async (req, supabase, authUserId, context) => {
  ...
  if (error) return err(error.message, 500);
  logAudit(req, authUserId, { action: "delete", entityType: "staff", entityId: id, description: "Deactivated staff member" }).catch(() => {});
  return ok(null);
});
