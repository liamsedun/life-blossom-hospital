...
export const PUT = withAuth(async (req, supabase, authUserId, context) => {
  ...
  const allowed = ["description", "category", "amount", "expense_date", "payment_method", "vendor", "notes", "bank_account_id"];
  ...
  logAudit(req, authUserId, { action: "update", entityType: "expense", entityId: id, description: "Updated expense", changes: updates }).catch(() => {});
  return ok(data);
});
...
export const DELETE = withAuth(async (req, supabase, authUserId, context) => {
  ...
  if (error) return err(error.message, 500);
  logAudit(req, authUserId, { action: "delete", entityType: "expense", entityId: id, description: "Deleted expense" }).catch(() => {});
  return ok(null);
});
