...
async function assertParticipant(
  ...
  const isRecipient = chat.recipient_user_id === authUserId;

  if (!isPatientOwner && !isStaffOwner && !isRecipient) throw new NotFoundError("Chat not found");
  return { chat, role };
}
  ...
  return withAuth(async (req, supabase, authUserId) => {
    ...
    if (role === "patient") {
      const { data: u } = await svc
        ...
        .single();
      other = u;
    } else if (chat.recipient_user_id) {
      // Staff-to-staff chat
      const otherUserId = chat.recipient_user_id === authUserId ? chat.staff_user_id : chat.recipient_user_id;
      const { data: u } = await svc
        .from("users")
        .select("id, first_name, last_name, role, avatar_url, phone")
        .eq("id", otherUserId)
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  return withAuth(async (req, supabase, authUserId) => {
    ...
    let recipientRole: string = "staff";
    let link: string = "";

    if (role === "patient") {
      recipientId = chat.staff_user_id;
      recipientRole = "staff";
      link = `/admin/chats/${chatId}`;
    } else if (chat.recipient_user_id) {
      // Staff-to-staff chat
      recipientId = chat.recipient_user_id === authUserId ? chat.staff_user_id : chat.recipient_user_id;
      recipientRole = "staff";
      link = `/admin/chats/${chatId}`;
    } else {
      recipientId = chat.patient?.user_id;
      recipientRole = "patient";
      link = `/patient/chats/${chatId}`;
    }
