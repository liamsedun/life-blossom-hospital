...
export const POST = withAuth(async (req, supabase, authUserId) => {
  ...
  const { data: payment } = await svc
    .from("payments")
    .select(
      "id, invoice_id, patient_id, amount, transaction_ref, method, invoice:invoices(invoice_number)"
    )
  ...
  const methodLabel = payment.method === "pos" ? "POS payment" : "bank transfer";
  ...
  // Send internal mail to the patient about the cancellation
  try {
    if (patient?.user_id && patient.user_id !== authUserId) {
      await sendInternalMessage(svc, {
        orgId,
        senderId: authUserId,
        recipientUserIds: [patient.user_id],
        subject: `Payment Declaration Cancelled — ${invText}`,
        body: `Your pending ${methodLabel} of ₦${Number(payment.amount || 0).toLocaleString()}${refText} for ${invText} could not be confirmed and has been cancelled.\n\nIf you completed this payment, please contact the hospital billing department for assistance.`,
      });
    }
  } catch (e) {
    console.error("[Cancel] failed to send internal message:", e);
  }
