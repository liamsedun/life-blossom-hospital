...
export const GET = withAuth(async (req, supabase, authUserId) => {
  ...
  for (const inv of invoices ?? []) {
    if (inv.status === "draft" || inv.status === "cancelled" || inv.status === "void" || inv.status === "refunded") continue;
    invoicedTotal += Number(inv.total || 0);
    invoiceCount += 1;
    if (inv.status === "pending" || inv.status === "partially_paid") {
      outstanding += Number(inv.total || 0) - Number(inv.paid_amount || 0);
    }
  }

  logView(req, authUserId, "billing_summary", null, `Viewed billing summary for ${month}`).catch(() => {});
  return ok({
    month,
