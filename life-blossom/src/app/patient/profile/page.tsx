            ...
            <div key={item.label} className="flex items-center gap-3 px-4 py-3.5 group">
                  ...
                  selectOptions[editingField] ? (
                    <div className="flex items-center gap-1 mt-0.5">
                      <select
                        ...
                        className="flex-1 h-8 text-sm bg-[#1a2540] border border-white/[0.15] rounded-lg px-2 text-white focus:outline-none focus:ring-2 focus:ring-[#e0a84a]/30 "
                        autoFocus
                      >
                      ...
                      <button onClick={cancelEdit} className="p-1 text-muted-foreground hover:text-foreground/70 shrink-0">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  ) : (
              ...
              {item.field && !editingField && (
                <button
                  ...
                  className="p-1.5 text-muted-foreground/60 hover:text-[#e0a84a] hover:bg-muted rounded-lg transition-all shrink-0 opacity-0 group-hover:opacity-100"
        ...
        <div className="divide-y divide-white/[0.04]">
          <div className="flex items-center justify-between px-4 py-3.5">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-foreground/80 font-medium">Notifications</span>
              {!supported && <span className="text-[10px] text-muted-foreground/60 ml-1">(unsupported)</span>}
              {permission === "denied" && <span className="text-[10px] text-rose-400 ml-1">(blocked)</span>}
            </div>
            <button
              onClick={async () => {
              ...
              className={cn(
                "relative inline-flex h-8 w-14 items-center rounded-full transition-colors duration-200 disabled:opacity-50",
          ...
          <div className="flex items-center justify-between px-4 py-3.5">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-foreground/80 font-medium">Dark Mode</span>
            </div>
            {toggleSwitch(darkMode, toggleDarkMode)}
          </div>
          <button
            onClick={() => setShowPasswordModal(true)}
            className="flex items-center justify-between w-full px-4 py-3.5 text-left hover:bg-muted/50 transition-colors group"
          >
            <div className="flex items-center gap-3">
              <Lock className="w-4 h-4 text-muted-foreground" />
              <span className="text-sm text-foreground/80 font-medium">Change Password</span>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-card backdrop-blur-xl border border-border rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              ...
              <button onClick={() => { setShowPasswordModal(false); setPwError(""); setPwSuccess(""); }} className="p-1 text-muted-foreground hover:text-foreground transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
