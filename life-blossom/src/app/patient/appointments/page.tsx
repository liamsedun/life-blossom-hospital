...
function GlassCard({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={cn(
      "relative rounded-2xl border border-border bg-card backdrop-blur-xl p-4 overflow-hidden transition-all duration-300 hover:border-white/[0.12]",
      className
    )}>
...
export default function AppointmentsPage() {
  ...
  const getInitials = (appt: Appointment) => {
    const doc = appt.doctor || appt.staff;
    if (doc?.user) {
      return `${doc.user.first_name.charAt(0)}${doc.user.last_name.charAt(0)}`.toUpperCase();
    }
    return "DR";
  };
  ...
  const getDoctorName = (appt: Appointment) => {
    const doc = appt.doctor || appt.staff;
    if (doc?.user) {
      return `${doc.user.first_name} ${doc.user.last_name}`;
    }
    return "Unassigned";
  };
  ...
  const getSpecialty = (appt: Appointment) => {
    const doc = appt.doctor || appt.staff;
    return doc?.specialization || doc?.department || "General";
  };

  const getPatientLabel = (appt: Appointment) => {
    if (appt.patient?.user) {
      const name = `${appt.patient.user.first_name} ${appt.patient.user.last_name}`;
      if (appt.patient.primary_account_id) {
        return `For: ${name} (Dependant)`;
      }
      return name;
    }
    return null;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-foreground">Appointments</h2>
        <span className="text-xs text-muted-foreground bg-muted px-2.5 py-1 rounded-full">{appts.length} total</span>
      </div>
      ...
      <div className="flex bg-muted rounded-xl p-1 border border-border">
        <button
          ...
          className={cn(
            ...
            tab === "upcoming" ? "bg-muted text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground/70"
          )}
        >
          Upcoming
        </button>
        <button
          ...
          className={cn(
            "flex-1 h-9 text-sm font-medium rounded-lg transition-all duration-200",
            tab === "past" ? "bg-muted text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground/70"
          )}
        >
          Past
        </button>
        ...
        {appts.length > 0 ? appts.map((appt) => (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{getSpecialty(appt)}</p>
                  </div>
                  <span
                    className={cn(
                      "text-[10px] font-semibold px-2 py-0.5 rounded-full border shrink-0",
                      statusColors[appt.status] || "bg-muted text-muted-foreground border-border"
                    )}
                  >
                    {statusLabels[appt.status] || appt.status}
                  </span>
                </div>
                {getPatientLabel(appt) && (
                  <p className="text-xs text-muted-foreground mt-1">{getPatientLabel(appt)}</p>
                )}
                {appt.reason && (
                  <p className="text-xs text-muted-foreground/70 mt-0.5 truncate">{appt.reason}</p>
                )}
                ...
                {tab === "upcoming" && (
                  <div className="flex gap-2 mt-3 pt-3 border-t border-border">
                    <button
                      ...
                      className="flex-1 h-9 text-xs font-medium rounded-xl border border-border text-foreground/70 hover:bg-muted transition-all"
                    >
                      Reschedule
                    </button>
                    <button
          ...
          <GlassCard>
            <div className="text-center py-6">
              <p className="text-sm text-muted-foreground">No {tab} appointments found.</p>
            </div>
          </GlassCard>
        )}
      </div>
      ...
      {rescheduleAppt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-card backdrop-blur-xl border border-border rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <h3 className="text-base font-semibold text-foreground">Reschedule Appointment</h3>
              <button onClick={() => setRescheduleAppt(null)} className="p-1 text-muted-foreground hover:text-foreground transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground block mb-1.5">New Date</label>
                <input
                  type="date"
                  value={rescheduleDate}
                  onChange={(e) => setRescheduleDate(e.target.value)}
                  className="w-full h-10 rounded-xl border border-border bg-muted px-3 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-[#e0a84a]/30 focus:border-[#e0a84a]/40"
                  required
                />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground block mb-1.5">New Time</label>
                <input
                  type="time"
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 h-11 rounded-xl border border-border text-sm font-medium text-foreground/70 hover:bg-muted transition-colors"
                >
                  Cancel
                </button>
                <button
      ...
      {cancelAppt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-card backdrop-blur-xl border border-border rounded-2xl shadow-2xl w-full max-w-xs p-6 text-center">
