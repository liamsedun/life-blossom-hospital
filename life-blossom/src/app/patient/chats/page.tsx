...
export default function PatientChatsPage() {
  ...
  const load = async (silent = false) => {
    ...
    try {
      ...
      if (json.success) {
        const sorted = { ...json.data, chats: [...(json.data.chats || [])].sort((a: any, b: any) => new Date(b.updated_at || b.last_message_at || b.created_at).getTime() - new Date(a.updated_at || a.last_message_at || a.created_at).getTime()) };
        setData(sorted);
      }
    } catch {
      /* ignore */
    ...
    <div className="min-h-[calc(100dvh-64px)]">
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full bg-muted border border-border rounded-xl pl-10 pr-4 py-3 text-sm text-foreground placeholder:text-muted-foreground/60 outline-none focus:border-[#e0a84a]/40 transition-colors"
        />
      </div>
        ...
        <div className="space-y-4">
          ...
          {chats.length > 0 && (
            <div className="space-y-2">
              <h2 className="text-[11px] uppercase tracking-wider text-muted-foreground/60 font-semibold px-1">Conversations</h2>
              {chats.map((chat) => (
                <Link
                  key={chat.id}
                  href={`/patient/chats/${chat.id}`}
                  className="flex items-center gap-3 bg-card border border-border rounded-2xl p-3 hover:bg-muted transition-colors"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={cn("text-xs truncate mt-0.5", chat.unread_count > 0 ? "text-foreground/80 font-medium" : "text-muted-foreground")}>
                      {chat.last_sender_id === user?.id ? "You: " : ""}
                      {chat.last_message || "Say hello 👋"}
                    </p>
                  </div>
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    <span className="text-[10px] text-muted-foreground/60">{chat.last_message_at ? chatListTime(chat.last_message_at) : ""}</span>
                    {chat.unread_count > 0 && (
          ...
          {directory.length > 0 && (
            <div className="space-y-2">
              <h2 className="text-[11px] uppercase tracking-wider text-muted-foreground/60 font-semibold px-1">Care team</h2>
              <div className="space-y-2">
                {directory.map((staff: ChatDirectoryEntry) => (
                  <button
                    ...
                    className="w-full flex items-center gap-3 bg-card border border-border rounded-2xl p-3 hover:bg-muted transition-colors text-left disabled:opacity-60"
                  >
          ...
          {chats.length === 0 && directory.length === 0 && !loading && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="text-foreground font-semibold">{search ? "No results found" : "No chats yet"}</h3>
