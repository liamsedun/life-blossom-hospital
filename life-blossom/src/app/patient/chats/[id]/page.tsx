...
export default function PatientChatWindowPage() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100dvh-180px)] text-muted-foreground">
  ...
  if (loadErr) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100dvh-180px)] text-center">
        <p className="text-foreground/60 text-sm mb-3">{loadErr}</p>
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-150px)] -mx-4">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-card border-b border-border shrink-0">
        <Link href="/patient/chats" className="p-1.5 -ml-1.5 rounded-lg hover:bg-muted transition-colors">
      ...
      >
        {hasMore && (
          <div className="flex justify-center py-2">
            <button onClick={loadOlder} className="text-[11px] text-muted-foreground hover:text-white/70 transition-colors flex items-center gap-1">
            ...
            <div key={msg.id}>
              {showDate && (
                <div className="flex justify-center py-3">
                  <span className="text-[10px] text-white/35 bg-white/[0.05] border border-border px-3 py-1 rounded-full">
                    {bubbleDate(msg.created_at)}
                  </span>
                </div>
              )}
              <div className={cn("flex", mine ? "justify-end" : "justify-start")}>
                <div className={cn("max-w-[78%] rounded-2xl px-3.5 py-2.5", mine ? "bg-gradient-to-br from-emerald-600 to-teal-700 text-white rounded-br-md" : "bg-[#1a2540] border border-border text-white/90 rounded-bl-md")}>
                  <p className="text-sm leading-snug whitespace-pre-wrap break-words">{msg.message}</p>
                  <div className={cn("flex items-center justify-end gap-1 mt-1", mine ? "text-foreground/60" : "text-muted-foreground/60")}>
      ...
      {/* Quick replies */}
      {showQuick && (
        <div className="px-4 py-2 bg-card border-t border-border shrink-0 overflow-x-auto flex gap-2">
          {QUICK_REPLIES.map((r) => (
            <button
              key={r}
              onClick={() => send(r)}
              className="text-xs text-foreground/80 bg-muted border border-border rounded-full px-3 py-1.5 whitespace-nowrap hover:bg-white/[0.1] transition-colors"
            >
              {r}
            </button>
          ))}
      ...
      <div className="flex items-center gap-2 px-4 py-3 bg-card border-t border-border shrink-0">
        <button
          ...
          className={cn("p-2 rounded-xl transition-colors", showQuick ? "bg-[#e0a84a]/20 text-[#e0a84a]" : "text-muted-foreground hover:bg-muted")}
          title="Quick replies"
        >
        ...
        <input
          ...
          className="flex-1 bg-white/[0.05] border border-border rounded-xl px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/60 outline-none focus:border-[#e0a84a]/40 transition-colors"
        />
        <button
          onClick={() => send()}
