...
export default function PatientInternalMailPage() {
  ...
  const loadMail = useCallback(async () => {
    ...
    try {
      ...
      if (j.success) {
        const sorted = (j.data || []).sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
        setMessages(sorted);
      }
    } catch {}
  ...
  const renderRow = (s: StaffUser, selected: string[], onToggle: (id: string) => void) => (
    <label key={s.id}
      ...
      <Avatar className="size-6">
        ...
        <AvatarFallback className="text-[9px] bg-muted text-foreground/60">
          {s.full_name?.split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase() || "?"}
        </AvatarFallback>
      </Avatar>
      <div className="flex-1 min-w-0">
        <p className="text-xs text-foreground/80 truncate">{s.full_name}</p>
      ...
      <div>
        <div className="relative">
          ...
          <Input value={search} onChange={(e) => onSearch(e.target.value)}
            ...
            className="pl-9 h-9 text-sm border-border bg-card text-foreground/90 placeholder:text-muted-foreground/60" />
        </div>
        {chips.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {chips.map((s) => (
              <span key={s.id}
                className="inline-flex items-center gap-1.5 pl-2 pr-1 py-1 rounded-full text-[11px] text-foreground/80 bg-[#e0a84a]/10 border border-[#e0a84a]/25">
                {s.full_name}
                <button type="button" onClick={() => onToggle(s.id)}
                  className="p-0.5 rounded-full text-muted-foreground hover:text-foreground hover:bg-white/[0.08]">
                  <X className="size-3" />
                </button>
              </span>
            ))}
        ...
        <div className="max-h-48 overflow-y-auto border border-border rounded-lg p-2 space-y-1 mt-2">
          {staff.length === 0 ? (
            <p className="text-xs text-muted-foreground/60 p-2">No recipients found</p>
  ...
  const inputClass = "h-9 text-sm border-border bg-card text-foreground/90 placeholder:text-muted-foreground/60";
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Your hospital mailbox</p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="border-border bg-card">
          <TabsTrigger value="inbox" className="text-foreground/60 data-[state=active]:text-foreground data-[state=active]:bg-muted gap-2">
            <Inbox className="size-4" /> Inbox
          </TabsTrigger>
          <TabsTrigger value="sent" className="text-foreground/60 data-[state=active]:text-foreground data-[state=active]:bg-muted gap-2">
            <Send className="size-4" /> Sent
          </TabsTrigger>
          <TabsTrigger value="compose" className="text-foreground/60 data-[state=active]:text-foreground data-[state=active]:bg-muted gap-2">
            <MessageSquare className="size-4" /> Compose
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="inbox" className="mt-3">
          {loading ? (
            ...
            <div className="flex flex-col items-center py-16 text-muted-foreground rounded-2xl border border-border bg-card">
              <MailOpen className="size-10 mb-3" />
              <p className="text-sm">No messages yet</p>
            </div>
          ) : (
            <div className="rounded-2xl border border-border bg-card overflow-hidden divide-y divide-white/[0.04]">
              {messages.map((msg) => (
                <div key={msg.recipient_row_id} className="flex items-center">
                  <button
                    onClick={() => openMessage(msg)}
                    className="flex-1 min-w-0 text-left px-4 py-3.5 active:bg-card transition-colors flex items-start gap-3"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <p className={cn("text-sm truncate", msg.is_read ? "text-foreground/70" : "text-foreground font-semibold")}>
                          {msg.subject}
                        </p>
                        <span className="text-[10px] text-muted-foreground/60 shrink-0">{timeAgo(msg.created_at)}</span>
                      </div>
                    ...
                    <ChevronRight className="size-4 text-foreground/20 shrink-0 mt-1.5" />
                  </button>
                  <button
                    onClick={() => handleDelete(msg, "inbox")}
                    title="Delete from inbox"
                    className="mr-3 p-2 rounded-lg text-muted-foreground/60 hover:text-red-400 hover:bg-red-500/10 transition-colors shrink-0"
                  >
        ...
        <TabsContent value="compose" className="mt-3">
          <Card className="border-border bg-card">
            <div className="p-4 space-y-4">
              <p className="text-sm font-medium text-foreground/80">Compose Message</p>

              <div>
                <label className="text-xs text-foreground/60 mb-1.5 block">TO:</label>
                <RecipientPicker
                  search={toSearch}
              ...
              <div>
                <label className="text-xs text-foreground/60 mb-1.5 block">CC:</label>
                <RecipientPicker
                  search={ccSearch}
              ...
              <div>
                <label htmlFor="subject" className="text-xs text-foreground/60 mb-1.5 block">Subject</label>
                <Input id="subject" value={subject} onChange={(e) => setSubject(e.target.value)}
                  placeholder="Message subject..." className={inputClass} />
              </div>

              <div>
                <label htmlFor="body" className="text-xs text-foreground/60 mb-1.5 block">Message</label>
                <Textarea id="body" value={body} onChange={(e) => setBody(e.target.value)}
                  placeholder="Type your message..."
                  rows={7}
                  className="border-border bg-card text-foreground/90 placeholder:text-muted-foreground/60 text-sm" />
              </div>
      ...
      {openMsg && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 pb-20">
          ...
          <div className="relative w-full sm:max-w-md max-h-[85vh] overflow-y-auto rounded-2xl border border-border bg-card p-5 text-foreground">
            ...
            <div className="mt-4 text-sm text-foreground/80 whitespace-pre-wrap leading-relaxed border-t border-border pt-4">
              {openMsg.body}
            </div>
