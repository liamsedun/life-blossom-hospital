...
export default function BookAppointmentPage() {
  ...
  useEffect(() => {
    fetch("/api/staff?role=doctor,radiographer,admin,lab_technician,pharmacist")
      .then((r) => r.json())
      .then((json) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <GlassCard className="space-y-5">
        {dependants.length > 0 && (
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Booking For</label>
              ...
              <button
                ...
                className={cn(
                  ...
                  patientLabel === "Self"
                    ...
                    : "border-border bg-card text-foreground/60 hover:border-[#e0a84a]/30"
                )}
              >
        ...
        <div className="relative">
          <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Department / Service</label>
          <button
            onClick={() => { setShowDept(!showDept); setShowDoc(false); }}
            className="w-full h-11 flex items-center justify-between px-3 border border-border bg-muted rounded-xl text-sm text-left hover:border-[#e0a84a]/40 transition-all"
          >
            <span className={department ? "text-foreground" : "text-muted-foreground/60"}>{department || "Select department"}</span>
            <ChevronDown className={cn("w-4 h-4 text-muted-foreground/60 transition-transform", showDept && "rotate-180")} />
          </button>
          {showDept && (
            <div className="absolute z-20 top-full mt-1 left-0 right-0 bg-card backdrop-blur-xl border border-border rounded-xl shadow-2xl max-h-48 overflow-y-auto">
              {departments.map((d) => (
                <button
                  key={d}
                  onClick={() => { setDepartment(d); setShowDept(false); }}
                  className="w-full text-left px-3 py-2.5 text-sm text-foreground/70 hover:bg-muted hover:text-foreground transition-colors"
                >
                  {d}
                </button>
              ))}
        ...
        <div className="relative">
          ...
          <button
            ...
            className="w-full h-11 flex items-center justify-between px-3 border border-border bg-muted rounded-xl text-sm text-left hover:border-[#e0a84a]/40 transition-all"
          >
            <span className={doctorId ? "text-foreground" : "text-muted-foreground/60"}>{doctorName || "Select staff member"}</span>
            <ChevronDown className={cn("w-4 h-4 text-muted-foreground/60 transition-transform", showDoc && "rotate-180")} />
          </button>
          {showDoc && (
            <div className="absolute z-20 top-full mt-1 left-0 right-0 bg-card backdrop-blur-xl border border-border rounded-xl shadow-2xl max-h-48 overflow-y-auto">
              {loadingDoctors ? (
                <div className="px-3 py-2.5 text-sm text-muted-foreground">Loading staff...</div>
              ) : doctors.length === 0 ? (
                <div className="px-3 py-2.5 text-sm text-muted-foreground">No staff available</div>
              ) : (
                <>
                  {department && (
                    <div className="px-3 py-1.5 text-[10px] font-medium text-muted-foreground uppercase tracking-wider border-b border-border">
                      All staff — showing regardless of department
                    </div>
                  )}
                  {doctors.map((d) => (
                    <button
                      key={d.id}
                      onClick={() => { setDoctorId(d.id); setDoctorName(`${d.user.first_name} ${d.user.last_name.charAt(0)}.`); setShowDoc(false); }}
                      className="w-full text-left px-3 py-2.5 text-sm hover:bg-muted transition-colors"
                    >
                      <span className="text-foreground">{`${d.user.first_name} ${d.user.last_name}`}</span>
                      <span className="text-muted-foreground text-[10px] ml-2 capitalize">{d.role.replace(/_/g, " ")}</span>
                    </button>
                  ))}
                </>
              )}
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full h-11 pl-9 pr-3 border border-border bg-muted rounded-xl text-sm text-foreground hover:border-[#e0a84a]/40 transition-all focus:outline-none focus:ring-2 focus:ring-[#e0a84a]/20 focus:border-[#e0a84a]/40"
            />
          </div>
        </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  timeSlot === slot.label
                    ...
                    : slot.available
                    ? "border-border text-foreground/60 hover:border-[#e0a84a]/30 hover:bg-muted"
                    : "border-white/[0.04] text-foreground/20 bg-white/[0.02] cursor-not-allowed"
                )}
              >
                <Clock className={cn("w-4 h-4 mb-1", timeSlot === slot.label ? "text-[#e0a84a]" : "text-muted-foreground")} />
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full px-3 py-2.5 border border-border bg-muted rounded-xl text-sm text-foreground placeholder:text-muted-foreground/60 resize-none hover:border-[#e0a84a]/40 transition-all focus:outline-none focus:ring-2 focus:ring-[#e0a84a]/20 focus:border-[#e0a84a]/40"
          />
        </div>
          ...
          <button
            ...
            disabled={!doctorId || !date || !timeSlot || saving}
