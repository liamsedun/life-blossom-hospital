      ...
      <div className="bg-card border border-border rounded-xl card-shadow p-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            {invoice.patient_id !== user?.patient?.id && invoice.patient?.user && (
              <p className="text-[10px] text-cyan-400 font-medium mt-0.5">
                Dependant: {invoice.patient.user.first_name} {invoice.patient.user.last_name}
              </p>
            )}
          </div>
          ...
          <div className="divide-y divide-border">
            {invoice.items.map((item) => (
              <div key={item.id} className="px-4 py-2.5 grid grid-cols-4 gap-2 text-sm">
                ...
                <span className="text-right font-semibold text-foreground">₦{item.line_total.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2 text-sm">
          ...
          {invoice.discount > 0 && (
            <div className="flex justify-between">
              ...
              <span className="text-danger">-₦{invoice.discount.toLocaleString()}</span>
            </div>
          )}
          {invoice.tax > 0 && (
            <div className="flex justify-between">
