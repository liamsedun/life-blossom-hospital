...
type LoginTab = "email" | "phone" | "patient_id";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const targetPath = role === "patient" ? "/patient" : "/admin";
  ...
  return (
    ...
    >
      ...
      <h1>Welcome back</h1>
      <p className="subtitle">Sign in to your patient portal</p>

      {/* Login type tabs */}
      <div className="auth-tabs">
        {(["email", "phone", "patient_id"] as const).map((tab) => (
          <button
            key={tab}
            type="button"
          ...
          >
            {tab === "patient_id" ? "Patient ID" : tab.charAt(0).toUpperCase() + tab.slice(1)}
          </button>
        ))}
      </div>
      ...
      {activeTab === "patient_id" && (
        <div className="auth-field">
          <input
            ...
            required
          />
          <label htmlFor="patientId">Patient / Dependant ID</label>
        </div>
      )}
      ...
      {/* Sign In button with door animation */}
